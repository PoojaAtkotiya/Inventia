                                ...
                                if ((a.Role.results.indexOf(t.Role) > -1) && a.UserSelection == true) {
                                    ...
                                    if (!IsNullOrUndefinedApprover(a.UserNameId) && a.UserNameId.results.length > 0) {
                                        if (t.ApproverId == null) {
                                            ...
                                            t.ApproverId = removeDuplicateFromArray(t.ApproverId);
                                        }
                                    }
                                }
...
function SaveLocalApprovalMatrix(sectionName, requestId, mainListName, isNewItem, mainListItem, approvalMatrixListName) {
                        ...
                        if (!IsNullOrUndefined(nextUsers)) {
                            if (nextApprover == '') {
                                ...
                                if (nextApprover.indexOf(nextUsers) == -1) {
                                    ...
                                    nextApprover = TrimComma(nextApprover.trim()) + "," + nextUsers;
                                }
                            }
                        }
                    });
    ...
    GetDataforMail(ListNames.MAINLIST, requestId);
...
function GetDataforMail(mainListName, listItemId) {
    AjaxCall(
        {
            url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('" + mainListName + "')/items(" + listItemId + ")",
            httpmethod: 'GET',
            calldatatype: 'JSON',
            async: false,
            headers:
            ...
            if (!IsNullOrUndefined(approvers)) {
                if (temp.Levels == nextLevel && temp.Status == ApproverStatus.PENDING) {
                    ...
                    if (isNewItem) {
                        ...
                        strContributer = TrimComma(strContributer.trim()) + "," + approvers;
                    }
                }
                ...
                //// else if (Convert.ToInt32(p.Levels) <= preLevel || (p.Levels == curLevel.ToString() && p.Status != ApproverStatus.PENDING))
                    ...
                    if (isNewItem) {
                        ...
                        strReader = TrimComma(strReader.trim()) + "," + approvers;
                    }
                }
            }
        });
